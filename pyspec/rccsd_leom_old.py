...
#        return update_amps(self, t1, t2, eris)
    ...
    def eomee_ccsd_singlet(self, nroots=1, left=False, koopmans=False, guess=None, diag=None):
        cput0 = (time.clock(), time.time())
        if diag is None:
        ...
        if left:
            matvec = self.leomee_ccsd_matvec_singlet
        else:
            matvec = self.eomee_ccsd_matvec_singlet
        eig = linalg_helper.eig
        if user_guess or koopmans:
            def pickeig(w, v, nr, envs):
                x0 = linalg_helper._gen_x0(envs['v'], envs['xs'])
            ...
            eee, evecs = eig(matvec, guess, precond, pick=pickeig,
                             tol=1e-8, max_cycle=self.max_cycle,
                             max_space=self.max_space, nroots=nroots,
                             verbose=self.verbose)
    ...
    def eomee_ccsd_triplet(self, nroots=1, left=False, koopmans=False, guess=None, diag=None):
        cput0 = (time.clock(), time.time())
        if diag is None:
        ...
        if user_guess or koopmans:
            ...
            eee, evecs = eig(self.eomee_ccsd_matvec_triplet, guess, precond, pick=pickeig,
                             tol=self.conv_tol, max_cycle=self.max_cycle,
                             max_space=self.max_space, nroots=nroots,
                             verbose=self.verbose)
        else:

            eee, evecs = eig(self.eomee_ccsd_matvec_triplet, guess, precond,
                             tol=self.conv_tol, max_cycle=self.max_cycle,
                             max_space=self.max_space, nroots=nroots,
                             verbose=self.verbose)
        ...
        for n, en, vn in zip(range(nroots), eee, evecs):
            ...
            logger.info(self, 'EOM-EE triplet root %d E = %.16g  qpwt = %.6g',
                        n, en, np.linalg.norm(t1)**2)
...
# (r1ba, r2abbb, r2bbab) = (-r1ab,-r2baaa, r2aaba)
    ...
    def amplitudes_to_vector2(self,t1, t2, out=None):
        nocc, nvir = t1.shape
        nov = nocc * nvir
    ...
    def vector_to_amplitudes2(self,vector, nmo = None, nocc = None):
    ...
    def amplitudes_to_vector(self, t1, t2, out=None):
        ...
        if self.pop is None:
            self.pop = self.get_pop(vector)
            #for i,j in self.pop:
            #    print vector[i],vector[j]
        vector = np.delete(vector,self.pop[:,1])
        return vector

    def vector_to_amplitudes(self, vector, nmo=None, nocc=None):
        if nocc is None: nocc = self.nocc
        if nmo is None: nmo = self.nmo
        for valpos,pos in self.pop:
            vector = np.insert(vector,pos,vector[valpos])
        nvir = nmo - nocc
        nov = nocc * nvir
        size = nov + nocc**2*nvir*(nvir+1)//2
        t1 = vector[:nov].copy().reshape((nocc,nvir))
    ...
    def get_pop(self, vector):
        ...
        for k in range(nvir):
            for i in range(nocc):
                for j in range(i):
                    pos1.append(init + skip1*i + skip2*j + (k+3)*k//2)
                    pos2.append(init + skip1*j + skip2*i + (k+3)*k//2)
