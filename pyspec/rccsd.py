...
#        return update_amps(self, t1, t2, eris)
    ...
    def eomee_ccsd_singlet(self, nroots=1, left=False, koopmans=False, guess=None, diag=None):
        ...
        if (nroots == diag.size):
            eig = np.linalg.eig
            H = np.zeros((nroots,nroots))
            for i in range(nroots):
                vec = np.zeros(nroots)
                vec[i] = 1.0
                if left:
                    H[:,i] = self.leomee_ccsd_matvec_singlet(vec)
                else:
                    H[:,i] = self.eomee_ccsd_matvec_singlet(vec)
            ...
            #if left:
                #H = scipy.sparse.linalg.LinearOperator((diag.size,diag.size), matvec = self.leomee_ccsd_matvec_singlet)
            #else:
                #H = scipy.sparse.linalg.LinearOperator((diag.size,diag.size), matvec = self.eomee_ccsd_matvec_singlet)
            
            if left:
                matvec = lambda xs: [self.leomee_ccsd_matvec_singlet(x) for x in xs]
            else:
            ...
            if user_guess or koopmans:
                def pickeig(w, v, nr, envs):
                    ...
                    idx = np.argmax( np.abs(np.dot(np.array(guess).conj(),np.array(x0).T)), axis=1 )
                    return w[idx].real, v[:,idx].real, idx
                #eee, evecs = eig(H,k = nroots, v0 = guess, which = 'SM', sigma = 0.1, maxiter = self.max_cycle, tol = self.conv_tol)
                conv, eee, evecs = eig(matvec, guess, precond, pick=pickeig,
                                       tol=self.conv_tol, max_cycle=self.max_cycle,
                                       max_space=self.max_space, nroots=nroots,
                                       verbose=self.verbose)
            else:
                #eee, evecs = eig(H, k = nroots, which = 'SM', sigma = 0.1, maxiter = self.max_cycle, tol = self.conv_tol)
    ...
    def get_r0(self,r_ee):
        nroots = len(r_ee)
        r0 = np.zeros(nroots)
        self.get_Hrow()
        Hr1, Hr2 = self.vector_to_amplitudes(self.Hrow)
        Hr1 = Hr1.reshape(-1)
        Hr2 = Hr2.reshape(-1)
        for i in range(nroots):
            r1, r2 = self.vector_to_amplitudes(r_ee[i])
            r1 = r1.reshape(-1)
            r2 = r2.reshape(-1)
            r0[i] = (np.dot(Hr1,r1) + np.dot(Hr2,r2))/self.eee[i]
